...
public class Solution {
    ...
    static int [][] map;
    static int[] isInject; // 주입 상태 확인하는 배열
    ...
    static void powerSet(int depth, int cnt){ // cnt : 주입 횟 수
        if(cnt >= result || cnt > K) return;
        // 기저 조건
        if(depth == D){
    ...
    static boolean check(){
        ...
        // 주입 정보 (isInject 배열과 각 행을 확인)
        columns: for (int j = 0; j < W ; j++) { // ---> 행을 반복해서 체크!
            // 각각의 열을 isInject과 비교하여 바꿔주고, 연속되는 k개가 있는지 확인
            // 연속되는 k개가 없다면, 바로 return false
            int tempCnt = 1; // 연속되는 숫자 확인
            for (int i = 1; i < D; i++) {
                // cur: 현재 상태 투입을 안했다면(-1) -> 그대로 / 투입 했다면 투입한 내용으로 변환
                int cur = isInject[i - 1] == -1 ? map[i - 1][j] : isInject[i - 1]; // 0 or 1
                // 다음 지점
                int next = isInject[i] == -1 ? map[i][j] : isInject[i]; // 0 or 1
